...
**Where this sits relative to published research (read this, it shapes the whole project):** in February 2026 the Budget Lab at Yale published a comparison of seven modern AI-exposure metrics and found they correlate at roughly 0.7 to 0.9, with disagreement concentrated in the most-exposed occupations. An ILO research brief (2026) showed that older automation measures and newer AI-capability measures point at different kinds of jobs. So "do the indices agree?" has a published first-order answer, and part of our September-October work is deliberately a **replication for calibration**: if our merged table reproduces the published agreement pattern, we know our pipeline is right. The new contribution, which nobody has published, is the rest: a clean crosswalked table that includes Frey-Osborne alongside the modern indices, an analysis of **which occupational features predict where the indices disagree**, and a per-occupation consensus tier with an agreement-confidence flag. Replicating a known result to validate your pipeline, then extending past it, is exactly how real research teams work.
